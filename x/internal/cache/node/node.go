// Package node is a node implementation with manual garbage collection support.
...
// A is an shared interface. This is our cache back-reference. We are
// defining this as an interface to avoid cyclic imports and to avoid a bloated
// module.
type A interface {
	Get(x cid.ID) (shared.N, bool)
	LeafSize() int
	K() vector.D
}
...
func (n *N) IsRoot() bool {
	...
	_, ok := n.shared.Get(n.ids[idParent])
	return !ok
}
...
func (n *N) IsLeaf() bool {
	...
	_, ok := n.shared.Get(n.ids[idLeft])
	return !ok
}
...
func (n *N) IsFull() bool {
	...
	return len(n.dataCache) >= n.shared.LeafSize()
}
...
func (n *N) Child(b branch.B) shared.N {
	if !n.IsAllocated() {
		panic("accessing an unallocated node")
	}
...
func (n *N) Parent() shared.N {
	if !n.IsAllocated() {
		panic("accessing an unallocated node")
	}

	m, ok := n.shared.Get(n.ids[idParent])
	if !ok {
		return nil
	}
	return m
...
func (n *N) Left() shared.N  { return n.Child(branch.BLeft) }
func (n *N) Right() shared.N { return n.Child(branch.BRight) }
